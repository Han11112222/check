...
def calculate_simulation(sim_len, sim_inv, sim_contrib, sim_other, sim_vol, sim_rev, sim_cost, 
    ...
    irr_reason = ""
    if net_inv <= 0:
        irr_reason = "초기 투자비 0원 이하(보조금 과다)로 수익률 산출 의미 없음"
    elif ocf <= 0:
...
# --------------------------------------------------------------------------
# [UI] 좌측 사이드바 (두 번째 사진의 설정창 구현)
# --------------------------------------------------------------------------
with st.sidebar:
    st.header("⚙️ 분석 변수 설정")
    st.subheader("📊 분석 기준")
    rate_pct = st.number_input("할인율 (%)", value=6.15, step=0.01, format="%.2f")
    tax_pct = st.number_input("세율 (%)", value=20.9, step=0.1, format="%.1f")
    period = st.number_input("상각 기간 (년)", value=30, step=1)
    
    st.subheader("💰 비용 단가 (2024년 기준)")
    # [수정] 대표님께서 요청하신 대로 이전 기준값을 디폴트로 세팅
...
if st.button("🚀 경제성 분석 실행", type="primary"):
    if sim_vol <= 0 or (sim_rev - sim_cost) <= 0:
        ...
        # [에러 해결] KeyError 방지를 위해 딕셔너리 내부 값을 정확히 호출
        ...
        3. **현금흐름**: 매년 **{res['ocf']:,.0f}원**의 **세후 수요개발 기대이익(적자)**이 발생하고 있습니다.
        ...
        if res['npv'] < 0:
            ...
            st.info(f"👉 연간 사용량이 **{res['required_vol']:,.0f} MJ**일 경우 최소 경제성 만족(NPV ≥ 0)이 가능합니다.")
        else:
            st.success(f"✅ 현재 연간 사용량({sim_vol:,.0f} MJ)은 경제성 확보 기준 충족!")
