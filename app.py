...
if df_plan is not None and not df_actual.empty:
    ...
    if not compare_df.empty:
        ...
        st.info(f"💡 **분석:** 신규 모델의 R²값이 **{r2_new:.3f}**로 도출되었습니다. 이는 요일/주차별 가중치 로직이 실제 수요 변화의 약 **{r2_new*100:.1f}%**를 정확히 설명하고 있음을 뜻합니다.")
        ...
        fig_gap.update_layout(title="오차가 0에 가까울수록 정밀한 모델입니다", barmode='group', xaxis_title="일자")
        ...
        fig_scatter.update_layout(xaxis_title="실제 공급량", yaxis_title="계획량")
        st.plotly_chart(fig_scatter, use_container_width=True)
    else:
        st.warning("비교 분석을 위한 실적 데이터가 부족합니다.")
else:
    st.info("데이터 로드 중입니다. 엑셀 파일명을 확인해주세요.")
